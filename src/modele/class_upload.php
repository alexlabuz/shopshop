            ...
            if(!empty($_FILES[$data]['name'])){
                if(!in_array(substr(strrchr($_FILES[$data]['name'], '.'), 1), $this->extensions) ){
                    ...
                    if(file_exists($_FILES[$data]['tmp_name']) && (filesize($_FILES[$data]['tmp_name'])) > $this->taille){
                        echo 'Votre fichier doit faire moins de '.($this->taille / 1000).' Mo ! votre fichier fait '. (filesize($_FILES[$data]['tmp_name'])/ 1000) . " Mo";
                    }
                    else{
                        ...
                        // stocke le chemin
                        $fichier["chemin"] = $this->chemin.'/'.$fichier['nom'];
                        // copie du fichier
                        move_uploaded_file($_FILES[$data]['tmp_name'], $fichier["chemin"]);
                    }
                }
            }
        }
